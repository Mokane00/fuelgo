...
const API = {
  ...
  admin: {
    ...
    createStation:      (data)      => req('POST',   '/stations', data),
    updateStation:      (id, d)     => req('PUT',    `/stations/${id}`, d),
    deleteStation:      (id)        => req('DELETE', `/stations/${id}`),
    ...
    loyaltyReport:      ()          => req('GET',    '/admin/reports/loyalty'),
    ratingsReport:      ()          => req('GET',    '/admin/reports/ratings'),
    transactionReport:  (from, to)  => req('GET',    `/admin/reports/transactions?from=${from}&to=${to}`),
